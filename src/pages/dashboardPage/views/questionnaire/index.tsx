...
const Questionnaire = () => {
  ...
  const { data, isLoading, isError } = useQuestionnaireApi({ popupId });

  const matched = matchQA(selectedQuestion, data?.surveys);
      ...
      {matched?.contents.map((content, idx) => (
        <div key={idx} className="flex items-center gap-2">
          ...
          <span className="flex gap-2">
            <span>{idx + 1}번</span>
            {isZeroTotal(matched) && (
              <span className="text-gray06">({content.selectedCount}명)</span>
            )}
          </span>
        </div>
      ))}
  ...
  const renderPieChart = () => {
    ...
    return (
      <div className="w-[1360px] h-[662px] bg-gray02 rounded-[50px] px-[60px] py-[45px]">
        <p className="font-semibold text-[36px]">{renderQuestionTitle()}</p>
        <div className="flex">
          <div className="w-[1240px] h-[490px] bg-gray01 mt-[40px] rounded-[40px] px-[40px] py-[30px] flex justify-between">
            <div className="flex flex-col gap-[20px] w-[458px]">
              {matched.contents.map((content, idx) => (
                <div
                  key={idx}
                  className="rounded-[20px] bg-gray02 text-[20px] flex items-center h-[72px] w-full"
                >
                    ...
                    <Tooltip
                      ...
                      content={(props: TooltipProps<number, string>) => {
                        if (!props.active || !props.payload?.length)
                          return null;
                        const target = props.payload[0].payload;
                        return (
                          <CustomTooltip
                            active={props.active}
                            payload={[
                              {
                                value: target.selectedCount,
                                name: target.title,
